...
MLP_CONFIG_1 = {'hidden_layer_sizes': (100,), 'activation': 'relu', 'max_iter': 5000}
MLP_CONFIG_2 = {'hidden_layer_sizes': (100, 50), 'activation': 'tanh', 'max_iter': 5000}
NF_CONFIG_1 = {'hidden_layer_sizes': (30,), 'activation': 'logistic', 'max_iter': 5000}
NF_CONFIG_2 = {'hidden_layer_sizes': (15, 10), 'activation': 'logistic', 'max_iter': 5000}

def perform_eda(df):
    print("--- Characterization and Exploratory Analysis ---")
    print(f"Problem: Auto MPG Regression")
    print(f"Number of samples: {df.shape[0]}")
    print(f"Number of features: {df.shape[1] - 1}")
    print(f"Task: Regression")
    print(f"Output Variable: mpg")
    
    plt.figure(figsize=(10, 6))
    sns.scatterplot(x='horsepower', y='mpg', data=df)
    plt.title("Horsepower vs MPG")
    plt.savefig("src/auto_mpg_scatter.png")
    plt.close()
...
def analyze_results(results):
    ...
    plt.title("R2 Comparison - Auto MPG")
    plt.savefig("src/auto_mpg_performance.png")
    plt.close()
